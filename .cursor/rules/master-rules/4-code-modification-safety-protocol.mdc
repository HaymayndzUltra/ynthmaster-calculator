...
alwaysApply: false
---
# Master Rule: Code Modification Safety Protocol — Windsurf Edition
...
3.  **Read the Latest Version:** Use the `read_file` tool to get the most current version of target file(s).
4.  **Verify Inconsistencies:** If file content contradicts recent conversation history, **STOP** and ask for clarification.
5.  **Apply Specific Rules:** Follow `1-context-discovery.md` to load relevant project-specific rules.
...
1. **Identify Connected Components:**
   - **Action:** Use `grep_search` or `code_search` tools.
...
2. **Analyze Function/Class Usages:**
   - **Action:** Use `grep_search` tool.
   - **Query:** `"{functionName}(" OR ".{methodName}("`
   - **Goal:** Map all usage points

3. **Identify Existing Tests:**
   - **Action:** Use `grep_search` tool with test file includes.
   - **Query:** `"{functionName}"` with `Includes: ["*test*", "*spec*"]`
...
**[STRICT]** For significant changes (>5 lines): Use `edit` or `multi_edit` tools with clear explanation
**[STRICT]** For new files: Always use `write_to_file` with full content
...
3. **Linting Validation:**
   - **Action:** Use `run_command` tool to execute linter on modified files.
