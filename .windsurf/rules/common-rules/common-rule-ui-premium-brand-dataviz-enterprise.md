...
## Integration Notes
- **Location**: Place these files in `.windsurf/rules/common-rules/` for monorepo-wide use, or in a project's `project-rules/` for targeted activation.
- **Activation**: These rules are triggered by keywords found in the `TRIGGERS` metadata.
- **Review**: Before committing, ensure the rule passes the Final Review Checklist from `.windsurf/rules/master-rules/6-how-to-create-effective-rules.md`.
